...
import { MaintenanceRecord } from './maintenanceStore';
...
interface FinanceState {
  ...
  addExpense: (expense: Omit<ExpenseRecord, 'id' | 'created_at'>, items: ExpenseItem[]) => Promise<{ error: any }>;
  updateExpense: (id: string, expense: Partial<ExpenseRecord>, items: ExpenseItem[]) => Promise<{ error: any }>;
...
export const useFinanceStore = create<FinanceState>((set, get) => ({
  ...
  fetchTransactions: async (motorcycleId?: string | null) => {
    ...
    try {
      ...
      const { data: maintData, error: maintError } = await maintQuery;
  ...
  addExpense: async (expense, items) => {
    ...
    try {
      ...
      const { data: newExpense, error: expError } = await insforge.database
        .from('expenses')
        .insert([expense])
        .select()
        .single();
