...
import { useFinanzasStore, UnifiedTransaction } from '@/store/finanzasStore';
...
export default function FinancesScreen() {
  ...
  const { transactions, isLoading, error, fetchTransactions, deleteExpense } = useFinanzasStore();
  ...
  const handleEdit = (id: string) => {
    router.push(`/finanzas/edit?id=${id}`);
  };
  ...
  const stats = useMemo(() => {
    ...
    transactions.forEach(t => {
      ...
      if (y && m && parseInt(y) === currentYear && parseInt(m) === currentMonth) {
        monthTotal += t.totalAmount;
        
        // Sum by category for the month
        if (t.type === 'MAINTENANCE') {
          categories['Mantenimiento'] = (categories['Mantenimiento'] || 0) + t.totalAmount;
        } else {
          (t.items || []).forEach(item => {
            const cat = item?.category || '';
            if (cat.includes('Combustible')) categories['Combustible'] = (categories['Combustible'] || 0) + (item?.amount || 0);
            else if (cat.includes('Seguro')) categories['Seguros'] = (categories['Seguros'] || 0) + (item?.amount || 0);
            else if (cat.includes('Repuesto')) categories['Repuestos'] = (categories['Repuestos'] || 0) + (item?.amount || 0);
            else categories['Otros'] = (categories['Otros'] || 0) + (item?.amount || 0);
          });
        }
      }
    });
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.horizontalScroll}>
          <View style={styles.summaryCard}>
            ...
            <Text style={styles.summaryCardAmount}>${(stats.categories['Combustible'] || 0).toLocaleString()}</Text>
          ...
          <View style={styles.summaryCard}>
            ...
            <Text style={styles.summaryCardAmount}>${(stats.categories['Mantenimiento'] || 0).toLocaleString()}</Text>
          ...
          <View style={styles.summaryCard}>
            ...
            <Text style={styles.summaryCardAmount}>${(stats.categories['Seguros'] || 0).toLocaleString()}</Text>
          ...
          <View style={styles.summaryCard}>
            ...
            <Text style={styles.summaryCardAmount}>${(stats.categories['Repuestos'] || 0).toLocaleString()}</Text>
        ...
        <View style={styles.filterSection}>
          ...
          <View style={styles.transactionsHeader}>
            ...
            <TouchableOpacity style={styles.addBtn} onPress={() => router.push('/finanzas/add')}>
      ...
      <TouchableOpacity
        ...
        activeOpacity={0.8}
        onPress={() => router.push('/finanzas/add')}
      >
