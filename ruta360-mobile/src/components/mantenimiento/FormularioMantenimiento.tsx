...
interface FormularioMantenimientoProps {
  initialData?: Partial<FormularioMantenimientoData>;
  isLoading: boolean;
  onSave: (data: FormularioMantenimientoData, totalCost: number) => void;
  submitLabel: string;
}
...
export default function FormularioMantenimiento({ initialData, isLoading, onSave, submitLabel }: FormularioMantenimientoProps) {
  const { motorcycles, activeMotorcycleId } = useVehiculoStore();
  
  const [formData, setFormData] = useState<FormularioMantenimientoData>({
    motorcycle_id: initialData?.motorcycle_id || activeMotorcycleId || '',
    services: initialData?.services && initialData.services.length > 0 
      ? initialData.services.map(s => ({ type: s.type || '', cost: (s.cost || 0).toString(), _key: Math.random().toString(36).substr(2, 9) }))
  ...
  const handleChange = (field: keyof FormularioMantenimientoData, value: any) => {
  ...
  const handleServiceChange = (index: number, field: keyof MaintenanceServiceInput, value: string) => {
    ...
    newServices[index] = { ...newServices[index], [field]: value } as MaintenanceServiceInput;
