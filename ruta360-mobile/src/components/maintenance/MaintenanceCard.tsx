...
import { MaintenanceRecord } from '@/store/maintenanceStore';
...
export default function MaintenanceCard({ record }: MaintenanceCardProps) {
  ...
  const handlePress = () => {
    router.push(`/maintenance/${record.id}/edit`);
  };
          ...
          <Text style={styles.title} numberOfLines={1}>
            {record.services && record.services.length > 0 
              ? record.services.length === 1 
                ? record.services[0].type 
                : `${record.services[0].type} y ${record.services.length - 1} más`
              : 'Servicio Múltiple'}
          </Text>
